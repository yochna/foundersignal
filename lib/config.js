...
// Active checkout provider: no PAN/business-registration gate to accept
// payments, since Lemon Squeezy is the Merchant of Record (they're legally
// the seller, we're their vendor) rather than a payment-aggregator-on-our-
// -own-legal-identity setup like Razorpay.
export const lemonsqueezy = {
  apiKey: str(process.env.LEMONSQUEEZY_API_KEY),
  storeId: str(process.env.LEMONSQUEEZY_STORE_ID),
  variantId: str(process.env.LEMONSQUEEZY_VARIANT_ID),
  webhookSecret: str(process.env.LEMONSQUEEZY_WEBHOOK_SECRET),
};
export const hasLemonsqueezy = Boolean(
  lemonsqueezy.apiKey && lemonsqueezy.storeId && lemonsqueezy.variantId
);
...
// Optional: powers the "Visitor analytics" card in /admin. Requires a Vercel
// access token (vercel.com/account/tokens) and Web Analytics enabled for this
// project in the Vercel dashboard. teamId is only needed for team projects.
export const vercelAnalytics = {
  token: str(process.env.VERCEL_ANALYTICS_TOKEN || process.env.VERCEL_TOKEN),
  projectId: str(process.env.VERCEL_PROJECT_ID),
  teamId: str(process.env.VERCEL_TEAM_ID),
};
export const hasVercelAnalytics = Boolean(vercelAnalytics.token && vercelAnalytics.projectId);
...
export function configSnapshot() {
  return {
    ...
    payments: { razorpay: hasRazorpay, lemonsqueezy: hasLemonsqueezy, configured: hasLemonsqueezy || hasRazorpay },
    analytics: { vercel: hasVercelAnalytics },
    sources: {
