...
const VALID_MOMENTUM = new Set(['rising', 'steady', 'declining']); function sanitizeMomentum(value) { const v = String(value || '').toLowerCase(); if (VALID_MOMENTUM.has(v)) return v; if (['surging', 'growing', 'accelerating', 'hot'].includes(v)) return 'rising'; if (['falling', 'cooling', 'shrinking', 'slowing'].includes(v)) return 'declining'; return 'steady'; }
...
function fromOpportunity(opp) {
  return {
    ...
    momentum: sanitizeMomentum(opp.momentum),
