...
function backfillFromSeed(candidate, seed) {
  ...
  // Deep-analysis objects (TAM/SAM/SOM, unit economics, buyer persona, GTM
  // playbook, verdict matrix). The model is not asked to reproduce these on
  // every run, so without this they would silently vanish from a card the
  // moment Gemini enriches its cluster, even though the seed brief had them.
  // "Empty" means every value in the object is a falsy/blank string, not just
  // that the key is missing.
  const OBJECT_FIELDS = ['verdictMatrix', 'tamAnalysis', 'unitEconomics', 'buyerPersona', 'gtmPlaybook'];
  const isBlankObject = (obj) =>
    !obj || typeof obj !== 'object' || Object.values(obj).every((v) => (Array.isArray(v) ? v.length === 0 : !String(v || '').trim()));
  for (const field of OBJECT_FIELDS) {
    if (isBlankObject(filled[field])) filled[field] = seed[field];
  }
